...
for sample_id in "${SAMPLE_IDS[@]}"; do
    cmd=(
        uv run python scripts/data/run_beat.py
        --config "${CONFIG_PATH}"
        --executor serial
        --stage samples
    ...
    job_id="$(slurm_submit_cpu_job "beat_${sample_id}" "" "${cmd[@]}")"
    job_id="${job_id%%;*}"
    JOB_IDS+=("${job_id}")
...
final_job_id="$(slurm_submit_cpu_job "beat_finalize" "${dependency}" "${final_cmd[@]}")"
