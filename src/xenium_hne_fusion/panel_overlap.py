...
def load_items_with_metadata_from_frame(items: pd.DataFrame, metadata_path: Path) -> pd.DataFrame:
    ...
    assert 'sample_id' in metadata.columns, 'metadata.sample_id is required'
...
def load_expr_genes(expr_path: Path) -> tuple[str, ...]:
    ...
    genes = tuple(name for name in names if name != TOKEN_INDEX_COLUMN)
    assert genes, f'No genes found: {expr_path}'
    return genes
...
def collect_sample_summaries(items: pd.DataFrame) -> list[dict]:
    ...
    for sample_id, group in sorted_items.groupby('sample_id', sort=True):
        ...
        for tile_dir in group['tile_dir']:
            expr_path = Path(tile_dir) / 'expr-kernel_size=16.parquet'
            genes = load_expr_genes(expr_path)
        ...
        summaries.append(
            {
                ...
                'signature_sizes': sorted({len(signature) for signature in signatures}),
                'genes': genes,
            }
        )
