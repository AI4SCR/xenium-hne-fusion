...
def test_supervised_parser_reads_yaml_config_into_namespace():
    ...
    assert data["data"]["items_path"] == Path("all.json")
    assert data["data"]["metadata_path"] == Path("default/outer=0-seed=0.parquet")
...
def test_supervised_namespace_bridge_returns_concrete_training_config():
    ...
    assert cfg.data.metadata_path == Path("default/outer=0-seed=0.parquet")
